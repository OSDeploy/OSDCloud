...
if ($AWZoneTextBox) {
	$AWZoneTextBox.Text = if ($global:OSDCloudInitialize.ApplicationWorkspaceZone) {
		[string]$global:OSDCloudInitialize.ApplicationWorkspaceZone
	} else {
		[string]::Empty
	}
}
...
if ($script:SelectionConfirmed) {
	...
	if ([string]::IsNullOrWhiteSpace($AWDeploymentTextBox.Text)) {
		$global:OSDCloudInitialize.ApplicationWorkspaceDeployment = $null
		$global:OSDCloudInitialize.ApplicationWorkspaceZone = $null
	} else {
		$global:OSDCloudInitialize.ApplicationWorkspaceDeployment = $AWDeploymentTextBox.Text.Trim()
	}
	...
	if ([string]::IsNullOrWhiteSpace($AWZoneTextBox.Text)) {
		...
		$global:OSDCloudInitialize.ApplicationWorkspaceZone = $AWZoneTextBox.Text.Trim()
	}
