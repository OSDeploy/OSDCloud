function step-validate-iswindowsimageready {
    ...
    if (-not ($global:OSDCloudWorkflowInvoke.ObjectOperatingSystem.FilePath)) {
        Write-Warning "[$(Get-Date -format s)] ObjectOperatingSystem does not have a Url to validate."
    ...
    try {
        $WebRequest = Invoke-WebRequest -Uri $global:OSDCloudWorkflowInvoke.ObjectOperatingSystem.FilePath -UseBasicParsing -Method Head
    ...
    $FileName = Split-Path $global:OSDCloudWorkflowInvoke.ObjectOperatingSystem.FilePath -Leaf
    $MatchingFiles = @()
