function Invoke-WpeinitPSModuleUpdate {
    [CmdletBinding()]
    param (
		...
		<component name="Microsoft-Windows-Setup" processorArchitecture="amd64"
			...
			<Run$($Run)>
				<Run$($Run)Command wcm:action="add">
					...
					<Path>powershell.exe -NoLogo -NoProfile -WindowStyle $WindowStyle $PSNoExit-Command Show-WinpeStartPSModuleUpdate -Name $Name</Path>
				</Run$($Run)Command>
			</Run$($Run)>
		</component>
